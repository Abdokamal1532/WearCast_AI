    ...
    def __call__(
        ...
        with torch.no_grad():
            ...
            if hasattr(self, '_cached_parse'):
                ...
                # Apply strong blur to the inpainted region to remove structural ghosts
                _ori_blurred = cv2.GaussianBlur(_ori_ctx_np, (51, 51), 0)
                _ori_ctx_np = np.where(inpaint_mask[:, :, None] > 0, _ori_blurred, _ori_ctx_np)
        ...
        with torch.no_grad():
            ...
            # This ensures we paste newly generated skin (to fill gaps left by old baggy clothes)
            # WITHOUT pasting the UNet's noisy fake background (which causes ghostly outlines).
            if hasattr(self, '_cached_parse'):
                ...
                # Erode the old torso slightly by 1px
                ...
                # Identify foreground generated by UNet (everything except background label 0)
                unet_not_bg = (parse_new > 0).astype(np.float32)
                unet_not_bg_t = torch.from_numpy(unet_not_bg).unsqueeze(0).unsqueeze(0).to(self.gpu_id)
                unet_not_bg_t = F.interpolate(unet_not_bg_t, size=(raw_generated.size[1], raw_generated.size[0]), mode='nearest')
                
                # Soften foreground mask slightly for smooth blending
                unet_not_bg_soft = kornia.filters.gaussian_blur2d(unet_not_bg_t, (5, 5), (1.0, 1.0))
                
                # Force paste ONLY the intersection
                force_mask = old_torso_soft * unet_not_bg_soft
                
                alpha_t = torch.maximum(alpha_t, force_mask)
        ...
        if hasattr(self, '_skin_mask'):
            ...
            # Removing the subtraction of 'alpha' ensures the skin texture is restored right up to the seam.
        ...
        # Laplacian Pyramid Blending for seamless edges
        print(" -> [COMPOSITE] Using Laplacian Pyramid Blending for seamless edges...")
        mask_soft = cv2.GaussianBlur((alpha * 255).astype(np.uint8), (15, 15), 0)
        final_image = self.laplacian_pyramid_blend(gen_arr, ori_final, mask_soft, levels=4)
